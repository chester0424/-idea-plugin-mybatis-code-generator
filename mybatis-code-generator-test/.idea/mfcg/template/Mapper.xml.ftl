...
<#list nonKeyFields as field>
            ${field.columnName} = ${r"#"}{${field.name},jdbcType=${field.jdbcType}}<#sep>,
</#list>
...
<#list nonKeyFields as field>
                <if test="key == '${field.name}'">
                    ${field.columnName} = ${r"#"}{value,jdbcType=${field.jdbcType}}
                </if>
</#list>
            </foreach>
        </set>
    ...
    <delete id="delete">
        DELETE FROM ${tableName} WHERE ${keyField.columnName} = ${r"#"}{${keyField.name}}
    </delete>
...
<#if ["char","String","Char"]?seq_contains(field.javaTypeShort)>
            ...
            <if test="${field.name}_array != null">
                ...
                <foreach collection="${field.name}_array" item="value" open="(" separator="," close=")">
                    ${r"#"}{value,jdbcType=${field.jdbcType}}
                </foreach>
            </if>
</#if>
...
${callback.setTargetFileName(className+"-mapper.xml")}